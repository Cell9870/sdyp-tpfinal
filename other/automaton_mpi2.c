#include <stdio.h>
#include "./headers/automaton_mpi2.h"
#include <math.h>
#include <stdlib.h>
#include <time.h>
...
int main (int argc, char** argv) {
    ...
    divideAndSendGrid(grid, gridAux, rank, size, mpi_node_type, myRows, myRange);
    //if (PERFORMANCE_MODE) printTimeSpent(&myBegin, rank, iteration, 0, &time_spent);

    while(iteration < ITERATIONS) {
        if (rank == 0 && !PERFORMANCE_MODE && (DEV_MODE==0)) {
            if (iteration == 0 || iteration == ITERATIONS-1) printf("Ciclo %d.\n", iteration+1);
            /*if (iteration < 3) {
                fflush(stdin);
                getchar();
            }*/
        ...
        MPI_Barrier(MPI_COMM_WORLD);
        if ((DEV_MODE==0) && !PERFORMANCE_MODE && rank == 0 && iteration == 0) showGrid(grid);
        ...
        updateGrid(grid, gridAux, rank, size, myRows, myRange, mpi_node_type, iteration);
        //if (PERFORMANCE_MODE) printTimeSpent(&myBegin, rank, iteration, 2, &time_spent);
        MPI_Barrier(MPI_COMM_WORLD);
        if ((DEV_MODE==0) && !PERFORMANCE_MODE && rank == 0 && iteration == ITERATIONS-1) showGrid(grid);
        MPI_Barrier(MPI_COMM_WORLD);
...
void initGrid(Node **grid) {
    for (int i = 0; i < HEIGHT; i++) 
        for (int j = 0; j < WIDTH; j++) {
            switch (INIT_TYPE) {
                case 1: {
                    grid[i][j].temperature = (((double) rand() / RAND_MAX) * 272 * 2) - 272;
                }; break;
                case 2: {
...
void updateRows(Node **grid, Node** gridAux, int rank, int size, MPI_Datatype MPI_NODE_TYPE, int myRows, int myRange[]) {
    MPI_Status status;
    
    for (int process = 0; process < size; process++) {
        if (process == rank+1) {
            if (DEV_MODE == -1 && process == 3) printf("up neigbor\n");
            for (int i = 0; i < ((MOORE_RANGE * 2) + myRows); i++) {
                if (inGrid(myRange[i], 0) && isMyDownRow(i, myRows)) {
                    if (DEV_MODE == -1 && process == 3) printf("%2d: sending row %2d to %2d. grid[%2d][0]: %11lf\n", rank, myRange[i], rank+1, myRange[i], gridAux[myRange[i]][0].temperature);
                    MPI_Send(gridAux[myRange[i]], WIDTH, MPI_NODE_TYPE, rank+1, 55, MPI_COMM_WORLD);
                }
        ...
        if (process == rank) {
            if (DEV_MODE == -1 && process == 3) printf("me\n");
            for (int i = 0; i < ((MOORE_RANGE * 2) + myRows); i++) {
                if (inGrid(myRange[i], 0) && isMyUpNeighborRow(i, myRows)) {
                    if (DEV_MODE == -1 && process == 3) printf("%2d: recving row %2d from %2d. grid[%2d][0]: %11lf antes\n", rank, myRange[i], rank-1, myRange[i], grid[myRange[i]][0].temperature);
                    MPI_Recv(grid[myRange[i]], WIDTH, MPI_NODE_TYPE, rank-1, 55, MPI_COMM_WORLD, &status);
                    if (DEV_MODE == -1 && process == 3) printf("%2d: recving row %2d from %2d. grid[%2d][0]: %11lf despues\n", rank, myRange[i], rank-1, myRange[i], grid[myRange[i]][0].temperature);
                }
                if (inGrid(myRange[i], 0) && isMyDownNeighborRow(i, myRows)) {
                    if (DEV_MODE == -1 && process == 3) printf("%2d: recving row %2d from %2d. grid[%2d][0]: %11lf antes\n", rank, myRange[i], rank+1, myRange[i], grid[myRange[i]][0].temperature);
                    MPI_Recv(grid[myRange[i]], WIDTH, MPI_NODE_TYPE, rank+1, 50, MPI_COMM_WORLD, &status);
                    if (DEV_MODE == -1 && process == 3) printf("%2d: recving row %2d from %2d. grid[%2d][0]: %11lf despues\n", rank, myRange[i], rank+1, myRange[i], grid[myRange[i]][0].temperature);
                }
        ...
        if (process == rank-1) {
            if (DEV_MODE == -1 && process == 3) printf("down neighbor\n");
            for (int i = 0; i < ((MOORE_RANGE * 2) + myRows); i++) {
                if (inGrid(myRange[i], 0) && isMyUpRow(i, myRows)) {
                    if (DEV_MODE == -1 && process == 3) printf("%2d: sending row %2d to %2d. grid[%2d][0]: %11lf\n", rank, myRange[i], rank-1, myRange[i], gridAux[myRange[i]][0].temperature);
                    MPI_Send(gridAux[myRange[i]], WIDTH, MPI_NODE_TYPE, rank-1, 50, MPI_COMM_WORLD);
                }
                if (inGrid(myRange[i], 0) && isMyRow(i, myRows)) {
                    for (int j = 0; j < WIDTH; j++) {
                        grid[myRange[i]][j] = gridAux[myRange[i]][j];
...
int isMyUpRow(int i, int myRows) {
    return (MOORE_RANGE <= i && i < (MOORE_RANGE*2));
}


int isMyUpNeighborRow(int i, int myRows) {
    return (0 <= i && i < MOORE_RANGE);
}

int isMyDownRow(int i, int myRows) {
    return (myRows <= i && i < (MOORE_RANGE + myRows));
}

int isMyDownNeighborRow(int i, int myRows) {
    return ((MOORE_RANGE + myRows) <= i && i < ((MOORE_RANGE*2) + myRows));
}

void divideAndSendGrid(Node **grid, Node **gridAux, int rank, int size, MPI_Datatype MPI_NODE_TYPE, int myRows, int myRange[]) {
    MPI_Status status;
...
void applyRulesGrid(Node **grid, Node **gridAux, int rank, int size, int myRows, int myRange[]) {
    ...
    if (DEV_MODE > 0) printMyRows(gridAux, rank, myRows, myRange, 4);
}
...
void updateGrid(Node **grid, Node **gridAux, int rank, int size, int myRows, int myRange[], MPI_Datatype MPI_NODE_TYPE, int iteration) {
    if (iteration < ITERATIONS-1) {
        updateRows(grid, gridAux, rank, size, MPI_NODE_TYPE, myRows, myRange);
    } 
    else {
        manualGatherGrid(grid, gridAux, rank, size, myRows, myRange, MPI_NODE_TYPE);
    }
}

void manualGatherGrid(Node **grid, Node **gridAux, int rank, int size, int myRows, int myRange[], MPI_Datatype MPI_NODE_TYPE) {
    MPI_Status status;
